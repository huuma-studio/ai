...
the rest while the command runs to its exit, and returns the kept prefix with
a `…[output truncated at 512 KiB]` note — a chatty command can neither exhaust
memory nor produce a tool result too large for the next model request, however
the bytes escape. Tools that can prompt or open a pager should
